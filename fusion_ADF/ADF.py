'''
    ADF是anisotropic diffusion fussion(各向异性扩散融合)
    paper: 《Fusion of Infrared and Visible Sensor Images Based on Anisotropic Diffusion and Karhunen–Loeve Transform》
...
if __name__ == '__main__':
    ...
    csv_file =  open('results_ADF.csv', 'w', encoding='gbk', newline="")
    ...
    for img_ids in os.listdir(path_rgb):
        ...
        fus_img = ADF(ira_id, rgb_id)
        # fus_img = ira_id
        cv2.imwrite('E://SIMpro//pro_infofusion//fusion_ADF//results//' + '{}_fusion.png'.format(img_id), fus_img)
        visrgb = rgb_id
        infra = ira_id
        fusion_last = fus_img
